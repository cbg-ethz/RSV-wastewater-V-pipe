...
# These are the lineages that were deconvolved
swiss_sub_lineages <- c("B.D", "B.D.1", "B.D.1.1", "B.D.4",
                        "B.D.4.1", "B.D.4.1.1", "B.D.E.1", "B.D.E.2", "B.D.E.4")
...
# select the main lineages found in wastewater B.D.E.1 and B.D.4.1.1
ww_combined <- ww_combined %>%
  filter(variant %in% c("B.D.E.1" , "B.D.4.1.1")) %>%
  group_by(variant, week, location) %>%
  # then sum - in this case it should not change anything
...
clin_ww_combined_zh <- merge(ww_combined_zurich, clinical_aggregated, by = "week", all.x = TRUE)
clin_ww_combined_ge <- merge(ww_combined_geneva, clinical_aggregated, by = "week", all.x = TRUE)
clin_ww_combined_zh <- clin_ww_combined_zh %>%
  rowwise() %>%
  mutate(
    delta_abs_BDE411 = abs(`B.D.4.1.1.x` - `B.D.4.1.1.y`),
    delta_abs_BDE1 = abs(`B.D.E.1.x` - `B.D.E.1.y`)
...
max_distance_zh <- max(clin_ww_combined_zh[,c("delta_abs_BDE411","delta_abs_BDE1")], na.rm = TRUE)
max_distance_ge <- max(clin_ww_combined_ge[,c("delta_abs_BDE411","delta_abs_BDE1")], na.rm = TRUE)

# Pivot longer

delta_long_zh <- clin_ww_combined_zh %>%
  dplyr::select(week, starts_with("delta_abs_BD"), total_counts) %>%
  pivot_longer(
    cols = starts_with("delta_abs_BD"),
    names_to = "lineage",
    values_to = "delta_abs"
  )
...
# Visualize
custom_labels <- c(
  "delta_abs_BDE1" = "B.D.E.1",
  "delta_abs_BDE411" = "B.D.4.1.1"
...
summary_zh <- delta_long_zh %>%
  ...
  summarise(
    ...
    min_delta  = min(delta_abs, na.rm = TRUE),
